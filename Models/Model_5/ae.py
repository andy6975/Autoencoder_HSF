...
with tf.variable_scope('Encoder'):
    ...
    encoder_1 = tf.layers.dense(X, 500, name='encoder_1', kernel_initializer=kernel_initializer, bias_initializer=bias_initializer)
    # encoder_1_bn = tf.layers.batch_normalization(encoder_1, training=is_training, momentum=0.9, name='encoder_1_bn')
    encoder_1_ac = tf.nn.relu(encoder_1, name='encoder_1_ac')

    encoder_2 = tf.layers.dense(encoder_1_ac, 250, name='encoder_2', kernel_initializer=kernel_initializer, bias_initializer=bias_initializer)
    # encoder_2_bn = tf.layers.batch_normalization(encoder_2, training=is_training, momentum=0.9)
    encoder_2_ac = tf.nn.relu(encoder_2)

    encoder_3 = tf.layers.dense(encoder_2_ac, 125, name='encoder_3', kernel_initializer=kernel_initializer, bias_initializer=bias_initializer)
    # encoder_3_bn = tf.layers.batch_normalization(encoder_3, training=is_training, momentum=0.9)
    encoder_3_ac = tf.nn.relu(encoder_3)

    encoder_4 = tf.layers.dense(encoder_3_ac, 60, name='encoder_4', kernel_initializer=kernel_initializer, bias_initializer=bias_initializer)
    # encoder_4_bn = tf.layers.batch_normalization(encoder_4, training=is_training, momentum=0.9)
    encoder_4_ac = tf.nn.relu(encoder_4)
    ...
    encoder_5_ac = tf.nn.relu(encoder_5)
    ...
    encoder_6_ac = tf.nn.relu(encoder_6)

with tf.variable_scope('Decoder'):

    decoder_1 = tf.layers.dense(encoder_6_ac, 30, name='decoder_1', kernel_initializer=kernel_initializer, bias_initializer=bias_initializer)
    # decoder_1_bn = tf.layers.batch_normalization(decoder_1, training=is_training, momentum=0.9)
    decoder_1_ac = tf.nn.relu(decoder_1)

    decoder_2 = tf.layers.dense(decoder_1_ac, 60, name='decoder_2', kernel_initializer=kernel_initializer, bias_initializer=bias_initializer)
    # decoder_2_bn = tf.layers.batch_normalization(decoder_2, training=is_training, momentum=0.9)
    decoder_2_ac = tf.nn.relu(decoder_2)

    decoder_3 = tf.layers.dense(decoder_2_ac, 125, name='decoder_3', kernel_initializer=kernel_initializer, bias_initializer=bias_initializer)
    # decoder_3_bn = tf.layers.batch_normalization(decoder_3, training=is_training, momentum=0.9)
    decoder_3_ac = tf.nn.relu(decoder_3)

    decoder_4 = tf.layers.dense(decoder_3_ac, 250, name='decoder_4', kernel_initializer=kernel_initializer, bias_initializer=bias_initializer)
    # decoder_4_bn = tf.layers.batch_normalization(decoder_4, training=is_training, momentum=0.9)
    decoder_4_ac = tf.nn.elu(decoder_4)
    ...
    decoder_5_ac = tf.nn.elu(decoder_5)
...
with tf.variable_scope('Loss'):
    mse_loss = tf.reduce_mean(tf.squared_difference(X, decoder_6))
...
with tf.Session() as sess:
    ...
    for epoch in range(1, EPOCHS+1):
        ...
        for batch in range(BATCHES):
            ...
            _, tr_l, s = sess.run([train_step, loss, merged_summary], feed_dict={X: x_main_tr, is_training: True})
            training_loss_ += tr_l
            writer.add_summary(s, epoch)

        df_val = df_val.sample(frac=1).reset_index(drop=True)
        val_l = loss.eval(feed_dict={X: df_val})
        p = decoder_6.eval(feed_dict={X: df_train})
    ...
    output_after_training = decoder_6.eval(feed_dict={X: df_test})
