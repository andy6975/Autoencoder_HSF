...
with tf.variable_scope('Encoder'):
    ...
    encoder_1 = tf.layers.dense(X, 200, name='encoder_1', kernel_initializer=kernel_initializer, bias_initializer=bias_initializer)
    ...
    encoder_2 = tf.layers.dense(encoder_1_ac, 100, name='encoder_2', kernel_initializer=kernel_initializer, bias_initializer=bias_initializer)
    # encoder_2_bn = tf.layers.batch_normalization(encoder_2, training=is_training, momentum=0.9)
    encoder_2_ac = tf.nn.tanh(encoder_2)

    encoder_3 = tf.layers.dense(encoder_2_ac, 50, name='encoder_3', kernel_initializer=kernel_initializer, bias_initializer=bias_initializer)
...
with tf.variable_scope('Decoder'):
    ...
    decoder_1 = tf.layers.dense(encoder_4_ac, 50, name='decoder_1', kernel_initializer=kernel_initializer, bias_initializer=bias_initializer)
    # decoder_1_bn = tf.layers.batch_normalization(decoder_1, training=is_training, momentum=0.9)
    decoder_1_ac = tf.nn.tanh(decoder_1)

    decoder_2 = tf.layers.dense(decoder_1_ac, 100, name='decoder_2', kernel_initializer=kernel_initializer, bias_initializer=bias_initializer)
    # decoder_2_bn = tf.layers.batch_normalization(decoder_2, training=is_training, momentum=0.9)
    decoder_2_ac = tf.nn.tanh(decoder_2)

    decoder_3 = tf.layers.dense(decoder_2_ac, 200, name='decoder_3', kernel_initializer=kernel_initializer, bias_initializer=bias_initializer)
...
with tf.Session() as sess:
    ...
    for epoch in range(1, EPOCHS+1):
        ...
        train_loss.append(training_loss_ / BATCHES)
...
print(residual_errors.min())

## Batch norm in last layer plus tanh
